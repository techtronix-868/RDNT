...
// THE FOLLOWING DEFINES IS FOR TMUX CONTROL AND MODE SEL BUTTON
...
static const char *ModeSelect_TAG = "MUX AND AUDIO";
// THE DEFINES FOR GPIO AND MODE SEL ENDS HERE
...
bool FFTtime = false;
bool bluetoothLedMode = false;
volatile int state;
// END OF GLOBAL STATE CONDITIONS
...
static void gatts_profile_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    ...
    case ESP_GATTS_WRITE_EVT: {
        ...
        if (!param->write.is_prep){
            ...
            bluetoothLedMode = (value == 48) ? false : true;
...
static void continuous_adc_init(adc_channel_t *channel, uint8_t channel_num, adc_continuous_handle_t *out_handle)
{
    ...
    adc_continuous_config_t dig_cfg = {
        .sample_freq_hz = 32 * 1000, // The value as per example was 20 made it 32.
...
void app_main(void)
{
    ...
    xTaskCreate(mode_switch, "mode_switch", 2048, NULL, 5, NULL);
    ...
    while (1)
    {
        ...
        while (1) {
            ...
            if (ret == ESP_OK) {
                ...
                {
                    //FFTOps();
                    if(state == 4 && bluetoothLedMode == false)
                    {
                        continue;
                    }
                    else
